...
module C1 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs__cudnn_attention_backward =
    foreign
      "atg__cudnn_attention_backward"
      (ptr raw_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> gc_tensor
  ...
  let stubs__flash_attention_forward_no_dropout_inplace =
    foreign
      "atg__flash_attention_forward_no_dropout_inplace"
      (gc_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> gc_tensor
  ...
  let stubs__fused_adagrad_tensor_lr_ =
    foreign
      "atg__fused_adagrad_tensor_lr_"
      (ptr gc_tensor
       @-> int
       @-> ptr gc_tensor
       @-> int
  ...
  let stubs__fused_adagrad_tensor_lr_out =
    foreign
      "atg__fused_adagrad_tensor_lr_out"
      (ptr gc_tensor
       @-> int
       @-> ptr gc_tensor
       @-> int
...
module C3 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs__philox_key_fold_in =
    foreign "atg__philox_key_fold_in" (gc_tensor @-> int64_t @-> returning raw_tensor)
  ;;

  let stubs__philox_key_split =
    foreign "atg__philox_key_split" (gc_tensor @-> int64_t @-> returning raw_tensor)
  ;;

  let stubs__philox_normal =
    foreign
      "atg__philox_normal"
      (gc_tensor @-> gc_tensor @-> double @-> double @-> returning raw_tensor)
  ;;

  let stubs__philox_normal_ =
    foreign
      "atg__philox_normal_"
      (gc_tensor @-> gc_tensor @-> double @-> double @-> returning raw_tensor)
  ;;

  let stubs__philox_normal_out =
    foreign
      "atg__philox_normal_out"
      (gc_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> double
  ...
  let stubs__philox_uniform =
    foreign
      "atg__philox_uniform"
      (gc_tensor @-> gc_tensor @-> double @-> double @-> returning raw_tensor)
  ;;

  let stubs__philox_uniform_ =
    foreign
      "atg__philox_uniform_"
      (gc_tensor @-> gc_tensor @-> double @-> double @-> returning raw_tensor)
  ;;

  let stubs__philox_uniform_out =
    foreign
      "atg__philox_uniform_out"
      (gc_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> double
  ...
  let stubs__scaled_grouped_mm_v2 =
    foreign
  ...
  let stubs__scaled_mm_v2_out =
    foreign
      "atg__scaled_mm_v2_out"
      (gc_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> ptr gc_tensor
...
module C4 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs__upsample_lanczos2d_aa =
    foreign
      "atg__upsample_lanczos2d_aa"
      (gc_tensor
       @-> ptr int64_t
       @-> int
       @-> int
...
module C5 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs__upsample_lanczos2d_aa_backward =
    foreign
      "atg__upsample_lanczos2d_aa_backward"
      (gc_tensor
       @-> ptr int64_t
       @-> int
       @-> ptr int64_t
  ...
  let stubs__upsample_lanczos2d_aa_backward_grad_input =
    foreign
      "atg__upsample_lanczos2d_aa_backward_grad_input"
      (gc_tensor
       @-> gc_tensor
       @-> ptr int64_t
       @-> int
  ...
  let stubs__upsample_lanczos2d_aa_out =
    foreign
      "atg__upsample_lanczos2d_aa_out"
      (gc_tensor
       @-> gc_tensor
       @-> ptr int64_t
       @-> int
  ...
  let stubs__upsample_lanczos2d_aa_vec =
    foreign
      "atg__upsample_lanczos2d_aa_vec"
      (gc_tensor
       @-> ptr int64_t
       @-> int
       @-> int
  ...
  let stubs__use_miopen_ctc_loss =
    foreign
  ...
  let stubs__use_miopen_ctc_loss_tensor =
    foreign
  ...
  let stubs__weight_int4pack_mm_with_scales_and_zeros =
    foreign
      "atg__weight_int4pack_mm_with_scales_and_zeros"
      (gc_tensor
       @-> gc_tensor
       @-> int64_t
       @-> gc_tensor
...
module C7 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_baddbmm_dtype_out =
    foreign
      "atg_baddbmm_dtype_out"
      (gc_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> gc_tensor
  ...
  let stubs_bmm_dtype =
    foreign "atg_bmm_dtype" (gc_tensor @-> gc_tensor @-> int @-> returning raw_tensor)
  ;;

  let stubs_bmm_dtype_out =
    foreign
      "atg_bmm_dtype_out"
      (gc_tensor @-> gc_tensor @-> gc_tensor @-> int @-> returning raw_tensor)
  ;;
...
module C9 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_dim = foreign "atg_dim" (gc_tensor @-> returning int64_t)
...
module C10 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_fbgemm_linear_fp16_weight_fp32_activation_out =
    foreign
      "atg_fbgemm_linear_fp16_weight_fp32_activation_out"
      (gc_tensor @-> gc_tensor @-> gc_tensor @-> gc_tensor @-> returning raw_tensor)
  ;;

  let stubs_fbgemm_linear_fp16_weight_out =
    foreign
      "atg_fbgemm_linear_fp16_weight_out"
      (gc_tensor @-> gc_tensor @-> gc_tensor @-> gc_tensor @-> returning raw_tensor)
  ;;
end
...
module C12 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_get_device = foreign "atg_get_device" (gc_tensor @-> returning int64_t)
...
module C13 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_hash_tensor =
    foreign
      "atg_hash_tensor"
      (gc_tensor @-> ptr int64_t @-> int @-> int @-> int64_t @-> returning raw_tensor)
  ;;

  let stubs_hash_tensor_out =
    foreign
      "atg_hash_tensor_out"
      (gc_tensor
       @-> gc_tensor
  ...
  let stubs_is_contiguous = foreign "atg_is_contiguous" (gc_tensor @-> returning bool)
...
module C17 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_miopen_ctc_loss_out =
    foreign
      "atg_miopen_ctc_loss_out"
      (ptr raw_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> gc_tensor
  ...
  let stubs_miopen_ctc_loss_tensor =
    foreign
      "atg_miopen_ctc_loss_tensor"
      (ptr raw_tensor
       @-> gc_tensor
       @-> gc_tensor
       @-> gc_tensor
  ...
  let stubs_mm_dtype =
    foreign "atg_mm_dtype" (gc_tensor @-> gc_tensor @-> int @-> returning raw_tensor)
  ;;

  let stubs_mm_dtype_out =
    foreign
      "atg_mm_dtype_out"
      (gc_tensor @-> gc_tensor @-> gc_tensor @-> int @-> returning raw_tensor)
  ;;
...
module C18 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_numel = foreign "atg_numel" (gc_tensor @-> returning int64_t)
...
module C19 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_randint_like_tensor =
    foreign "atg_randint_like_tensor" (gc_tensor @-> gc_tensor @-> returning raw_tensor)
  ;;

  let stubs_randint_like_tensor_out =
    foreign
      "atg_randint_like_tensor_out"
      (gc_tensor @-> gc_tensor @-> gc_tensor @-> returning raw_tensor)
  ;;
...
module C24 (F : Cstubs.FOREIGN) = struct
  ...
  let stubs_storage_offset = foreign "atg_storage_offset" (gc_tensor @-> returning int64_t)
